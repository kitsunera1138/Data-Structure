...
public:
	...
	void Insert(T data) {
		if (size >= SIZE) {
			cout << "Adjacency Matrix Overflow";
		}
		else {
			vertex[size++] = data;
		}
...
{
#pragma region 그래프
	//요소들이 서로 복잡하게 연결되어 있는 관계를 표현하는 자료구조이며, 
	// 정점과 간선들의 집합으로 구성되어 있습니다.

	// 정점 (Vertex)
	// 노드 (Node) 데이터가 저장되는 그래프의 기본 원소 입니다.
...
#pragma region 인접 행렬
	...
	// 정점의 개수가 V일 때, V * V(V x V) 크기의 2차원 배열을 사용합니다.
